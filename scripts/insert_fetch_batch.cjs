...
const fetchBatchDataDef = `  const fetchBatchData = async (taskIds: string[]) => {
...
code = code.replace(`  const fetchTasks = async (reset: boolean = false) => {`, fetchBatchDataDef);

fs.writeFileSync('src/pages/Tasks.tsx', code);
console.log("Successfully inserted fetchBatchData.");

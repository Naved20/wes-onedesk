-- 1. Backfill NULL calculated_status on existing rows
UPDATE public.attendance
SET calculated_status = CASE
  WHEN status = 'holiday' THEN 'holiday'
  WHEN status = 'rejected' THEN 'absent'
  WHEN is_half_day THEN 'half_day'
  WHEN is_late THEN 'late'
  WHEN check_in_time IS NOT NULL OR status IN ('approved','present') THEN 'present'
  ELSE NULL
END
WHERE calculated_status IS NULL
  AND (status IS NOT NULL OR check_in_time IS NOT NULL);

-- 2. Helper: normalized effective status for a single attendance row
...
-- 3. Rewrite the monthly stats RPC (NULL safe, no double counting, real absent counts)

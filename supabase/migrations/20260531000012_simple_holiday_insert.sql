-- Simple Holiday Insert - Without ON CONFLICT issues

-- Step 1: Ensure ENUM has 'holiday' status
DO $$
BEGIN
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'ENUM already has holiday status: %', SQLERRM;
END $$;

-- Step 2: Create generate_sundays_for_year function
...
SELECT sunday_date, 'Sunday', 'Weekly Off', true
FROM generate_sundays_for_year(2026)
WHERE NOT EXISTS (SELECT 1 FROM holidays WHERE date = sunday_date)
ON CONFLICT (date) DO NOTHING;
...
-- Step 5: Insert holiday attendance records - WITHOUT ON CONFLICT
-- Delete existing holiday records first to avoid conflicts
DELETE FROM attendance WHERE status = 'holiday';

-- Now insert fresh holiday records
INSERT INTO attendance (user_id, date, status, created_at)
SELECT 
  ep.user_id,
  h.date,
  'holiday'::attendance_status,
  NOW()
FROM holidays h
CROSS JOIN (
...
-- If no active employees, insert for all auth users
INSERT INTO attendance (user_id, date, status, created_at)
SELECT 
  u.id,
  h.date,
...
WHERE NOT EXISTS (
  ...
  WHERE user_id = u.id AND date = h.date
);

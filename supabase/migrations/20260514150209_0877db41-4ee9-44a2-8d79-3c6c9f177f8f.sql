...
-- Function to sync approved leave to attendance records
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS DISTINCT FROM 'approved') THEN
    ...
    IF NEW.leave_type IN ('casual', 'emergency') THEN
      v_calc_status := 'paid_leave';
    ELSIF NEW.leave_type = 'sick' THEN
      v_calc_status := 'half_day';
    ...
    v_is_half := COALESCE(NEW.is_half_day, false) OR (NEW.leave_type = 'sick');
...
-- Ensure unique (user_id, date) on attendance for ON CONFLICT
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    -- Remove duplicates first if any (keep latest)
...
-- Also sync newly inserted leaves that come in already approved
...
-- Backfill: sync all currently approved leaves to attendance
DO $$
DECLARE
  r RECORD;
  v_current DATE;
...
BEGIN
  FOR r IN SELECT * FROM leaves WHERE status = 'approved' LOOP
    IF r.leave_type IN ('casual', 'emergency') THEN
      v_calc_status := 'paid_leave';
    ELSIF r.leave_type = 'sick' THEN
      v_calc_status := 'half_day';
    ELSIF r.leave_type = 'unplanned' THEN
      v_calc_status := 'leave';
    ELSE
      v_calc_status := 'leave';
    END IF;

    v_is_half := COALESCE(r.is_half_day, false) OR (r.leave_type = 'sick');

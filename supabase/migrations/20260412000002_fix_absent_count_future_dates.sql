-- Fix absent count to only count actual absent records, not calculate from working days
-- This prevents counting future dates as absent
...
BEGIN
  ...
  -- FIXED: Count actual absent records from database instead of calculating
  -- This prevents counting future dates as absent
  -- Count full day absents + half day absents (0.5 each)
  SELECT COALESCE(
  ...
  WHERE user_id = p_user_id
    ...
    AND date <= CURRENT_DATE;  -- Only count past and today's absents
...
'Calculates attendance statistics for a user in a specific month. Fixed to only count actual absent records up to current date, preventing future dates from being counted as absent.';
